...
function createChallengeLinkElement(data, parent) {
	...
	expires.textContent = "Instance ends in " + calculateExpiry(new Date(data.expires)) + " minutes.";
...
function view_container_info(challenge_id) {
    ...
    .then(data => {
        if (data.status == "Instance has not started") {
...
function container_stop(challenge_id) {
    ...
    .then(data => {
        if (data.error !== undefined) {
            ...
            // Success
            alert.append("Instance stopped successfully.");
            toggleChallengeCreate();
            toggleChallengeUpdate();
        }
    })
